...
class WorkflowManager:
    ...
    def get_workflow_catalog(self) -> list[Dict[str, Any]]:
        ...
        for workflow_path in sorted(self.workflows_dir.glob("*.json")):
            ...
            if not workflow_defaults and workflow_id in [
                "generate_image",
                "generate_song",
                "generate_video",
            ]:
    ...
    def apply_workflow_overrides(
        ...
        for param_name, value in overrides.items():
            if param_name not in override_mappings:
                logger.warning(
                    f"Override '{param_name}' not in declared mappings for {workflow_id}, skipping"
                )
                continue
            ...
            if param_name in constraints:
                ...
                if "enum" in constraint and value not in constraint["enum"]:
                    raise ValueError(
                        f"Value '{value}' for '{param_name}' not in allowed enum: {constraint['enum']}"
                    )
                if "min" in constraint and value < constraint["min"]:
                    raise ValueError(
                        f"Value '{value}' for '{param_name}' below minimum: {constraint['min']}"
                    )
                if "max" in constraint and value > constraint["max"]:
                    raise ValueError(
                        f"Value '{value}' for '{param_name}' above maximum: {constraint['max']}"
                    )
        ...
        for param_name, param in parameters.items():
            if param_name not in overrides and not param.required:
                if defaults_manager:
                    default_value = defaults_manager.get_default(
    ...
    def _load_workflows(self):
        ...
        for workflow_path in sorted(self.workflows_dir.glob("*.json")):
            try:
                ...
                logger.error(
                    "Skipping workflow %s due to JSON error: %s",
                    workflow_path.name,
                    exc,
                )
                continue
            ...
            tool_name = self._dedupe_tool_name(
                self._derive_tool_name(workflow_path.stem)
            )
        ...
        for param in definition.parameters.values():
            ...
            if raw_value is None:
                if param.name == "seed" and param.annotation is int:
                    ...
                    raw_value = defaults_manager.get_default(
                        namespace, param.name, None
                    )
                    if raw_value is not None:
                        logger.debug(
                            f"Using default value for {param.name}: {raw_value}"
                        )
                    else:
    ...
    def _extract_parameters(self, workflow: Dict[str, Any]):
        ...
        # Helper function to recursively search for PARAM_* placeholders
        def search_nodes_for_params(nodes_dict, node_prefix=""):
            nonlocal parameters
            if not isinstance(nodes_dict, dict):
                return

            for node_id, node in nodes_dict.items():
                # Skip non-dict nodes and metadata keys
                ...
                for input_name, value in inputs.items():
                    ...
                    if not parameter:
                        ...
                        optional_params = {
                            ...
                            "negative_prompt",
                            "seconds",
                            "lyrics_strength",  # Audio-specific optional params
                            "duration",
                            "fps",  # Video-specific optional params
                        }
        ...
        search_nodes_for_params(workflow)

        # Also search within definitions (subgraphs) for workflows like generate_image_zai.json
        if "definitions" in workflow and isinstance(workflow["definitions"], dict):
            for def_id, definition in workflow["definitions"].items():
                if isinstance(definition, dict) and "nodes" in definition:
                    # Search in subgraph nodes
                    search_nodes_for_params(definition["nodes"])
